...
- 🆘 Тикеты поддержки с ответственным сотрудником, ответами и закрытием.
...
- 🔎 Поиск `/find QUERY`; массовые команды `/bulk_disable PREFIX`,
  `/bulk_enable PREFIX`, `/bulk_extend PREFIX 30d`.
- 📣 Подтверждаемая рассылка всем пользователям.
- 💾 Ежедневные SQLite-копии в `/var/lib/awgram/backups`.
- 🚨 Мониторинг AmneziaWG каждые пять минут и аудит операций.

Роль сотрудника назначает владелец командой
`/role TELEGRAM_ID technical|support|finance|remove`.
