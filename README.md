...
В базе можно безопасно подготовить партнёра, его наценку и покупателей; токен
дочернего Telegram-бота в SQLite не хранится — только путь к отдельному
защищённому secret-файлу. Запуск дочерних ботов и расчёт взаиморасчётов будут
включаться последующими версиями после настройки партнёра владельцем сервиса.
